...
function populateTable(object, i){
    ...
    table += `<td><a type='button' class='' data-bs-toggle='modal' data-bs-target='#spostaCartella' onclick = saveObjectId('${object.id}') ><i class='bi bi-box-arrow-in-up-right'></i></a></td>`;
...
function popUpMobile(object, i){
    ...
    if(object.get("fileType") === 'Immagine')
        popup += "<a href = '" + linkImg + "' target = '_blank'><img class= 'rounded mx-auto d-block' style = 'height: 200px;' src = '" + linkImg + "'></a>";
    else 
...
async function printFolders(folder){
    ...
    if(results.length !== 0 || results2.length !== 0) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            str += "<td>" + object.get("updatedAt").toString().substring(4, 15) + "</td>";
...
// ------------------------------------- Viene chiamata quando viene cliccata la cartella (viene aperta)
